...
app.get('/saveImages', async (req, res) => {
    ...
    try {
        ...
        if (response.status === 200) {
			...
			const imgdirectory = path.join(__dirname, 'images_imx');
            if (!fs.existsSync(imgdirectory)) {
                fs.mkdirSync(imgdirectory);
			}
            const subdirectory = path.join(__dirname, 'images_imx', currentAlphanumeric);
            ...
            for (let i = 0; i < imgSrcList.length; i++) {
                ...
                const imageUrl = imgSrcList[i].replace('<img src="', '').replace('"', '').replace('to/u/t', 'to/u/i');
				...
				process.stdout.write("\rdl image " + (i+1) + " of " + imgSrcList.length + "    ");
...
app.get('/getImages', async (req, res) => {
    ...
    const configFilePath = path.join(__dirname, 'config', 'config_imx.csv');
...
function extractImageSrcList(html) {
    ...
    const imgSrcList = [];
    const imgRegex = /<img src="https:..imx.to.u.t([^">]+)"/g;
...
function extractImageSrc(html) {
    const imgSrcList = [];
    const imgRegex = /<img src="https:..imx.to.u.t([^">]+)"/;
    //const imgRegex = /w.dth/g;
    let match;
...
function saveLogToCSV(currentInteger, currentAlphanumeric, url, title, numImages) {
    ...
    const logFilePath = path.join(__dirname, 'logs', 'log_imx.csv');
